# Skill de Colaboración para IA (Claude Code / Codex)

> **Audiencia:** Eres la IA asistente de cofundadores que no saben git ni programación.  
> **Objetivo:** Que puedan trabajar en el mismo código sin pisarse, usando tags y versionado con números.  
> **Estilo:** Inspirado en framework-charlie (un solo tag por feature terminado).
...
- Hacé commits **frecuentes** con mensajes descriptivos. No esperes al final del día.
...
4. **Calcular la nueva versión (tag):**
   - Leé el último tag: `git describe --tags --abbrev=0`
   - Si no hay tags, empezá en `v0.1.0`.
   - Reglas de incremento (semVer simple):
     - **patch** (`v0.1.0` → `v0.1.1`): arreglo chico, ajuste, corrección.
     - **minor** (`v0.1.1` → `v0.2.0`): nuevo feature, funcionalidad nueva.
     - **major** (`v0.2.0` → `v1.0.0`): cambio grande, reescritura, o cuando el humano diga "versión nueva importante".
   - Ejemplo práctico: si el último tag es `v0.1.2` y acaban de terminar un nuevo feature → el nuevo tag es `v0.2.0`.
...
6. **`git push origin main && git push origin vX.Y.Z`** — subí `main` y el tag.
   ...
   - Si el push falla porque `main` cambió (el otro cofundador subió algo mientras tanto), hacé `git pull origin main`, resolvé lo que sea necesario, y volvé a intentar push.
...
7. **Confirmá al humano con un resumen claro:**
   ```text
   ✅ Feature cerrado.
   - Commit: feat: descripción del feature
   - Tag: v0.2.0
   - Subido a main.
   ```
...
## Glosario mínimo para el humano

Cuando tengas que explicar algo, usá estas frases en vez de jargon:

- **"Actualizar"** = traer los cambios del otro (`git pull`)
- **"Guardar versión"** = commit + tag (`git commit` + `git tag`)
- **"Subir"** = push (`git push`)
...
- Si el humano dice "empecé" y no estás seguro de si ya hiciste los pasos de "ANTES", ejecutalos igual. Mejor verificar dos veces que pisar trabajo.
- Siempre terminá dejando al humano en la rama `main`.
- Si el repo está recién clonado y no tiene rama `main`, creala con `git checkout -b main`.
