...
// Prioridad: conv.Models[fw] (override) > manifest.Model > fallback > default.
// Siempre devuelve un spec válido o error crítico.
func modelSpecFor(conv *Conversation, frameworkName string) (llm.Spec, error) {
	man, err := loadFrameworkManifest(frameworkName)
	if err != nil {
		return llm.Spec{}, fmt.Errorf("no se pudo cargar manifest de %s: %w", frameworkName, err)
	}

	// Spec base del manifest
	spec := llm.Spec{
	...
	// Override por variable de entorno global
	...
	// Override por conversación (frontend)
	if override, ok := conv.Models[frameworkName]; ok && override != "" {
		spec.Name = override
	}

	// Verificar que tenemos API key para el provider seleccionado
	if os.Getenv(spec.EnvKey) == "" {
		// Si no hay key para el primario, intentar fallback
		if man.Model.Fallback.Provider != "" && os.Getenv(man.Model.Fallback.EnvKey) != "" {
			fmt.Fprintf(os.Stderr, "[modelSpecFor] %s: fallback de %s a %s (no hay %s)\n",
				frameworkName, spec.Provider, man.Model.Fallback.Provider, spec.EnvKey)
			spec.Provider = man.Model.Fallback.Provider
			spec.Name = man.Model.Fallback.Name
			spec.EnvKey = man.Model.Fallback.EnvKey
			spec.BaseURL = ""
		} else {
			return llm.Spec{}, fmt.Errorf("no hay API key para %s (env: %s) y no hay fallback disponible",
				spec.Provider, spec.EnvKey)
		}
	}
...
func preprocessVision(ctx context.Context, conv *Conversation, targetFramework string, userText string, resources []MessageResource) (string, error) {
	...
	if err := appendImageAnalysis(conv, targetFramework, userText, imgs, cleaned); err != nil {
		fmt.Fprintf(os.Stderr, "[api_rest] warn: no pude persistir image_analysis: %v\n", err)
	}
