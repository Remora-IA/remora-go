#!/usr/bin/env bash
# Smoke test end-to-end de la API REST api_rest conectada a Channel.
#
# Levanta Channel + api_rest, crea una conversación con frameworks=[echo,alfa],
...
#
# Uso:
#   bash scripts/smoke_test_api_rest.sh
#
# Requiere: jq, curl, go.
...
echo "── 2) Levantando api_rest en :$API_PORT ──"
cd "$REPO_ROOT/remora-flujo"
CHANNEL_URL="http://localhost:$CHANNEL_PORT" CHANNEL_API_KEY="$API_KEY" API_REST_PORT="$API_PORT" \
  go run ./cmd/api_rest &
API_PID=$!
sleep 6
