...
func (g *genericDriver) IngestAnswer(ctx context.Context, ch *adapter.Client, conv *Conversation, qctx QueuedAnswerCtx) error {
	...
	if commandHasParam(cmd, "business_id") {
		params["business_id"] = conversationBusinessID(conv)
	}
	...
	if resp.ExitCode != 0 {
		errDetail := strings.TrimSpace(resp.Stderr)
		if errDetail == "" {
			errDetail = strings.TrimSpace(resp.Stdout)
		}
		return fmt.Errorf("%s ingest-answer exit %d: %s", g.manifest.Name, resp.ExitCode, errDetail)
	}
	return nil
}
...
func conversationBusinessID(conv *Conversation) string {
	if conv != nil && conv.BusinessID != "" {
		return conv.BusinessID
	}
	return ""
}

func normalizeRuntimeContext(businessID string, ctx map[string]any) map[string]any {
	out := map[string]any{}
	for k, v := range ctx {
		out[k] = v
	}
	...
	if _, ok := out["audience"]; !ok {
		out["audience"] = "collector"
	}
	return out
}

func encodeConversationRuntimeContext(conv *Conversation) string {
	if conv == nil {
		return ""
	}
