...
func (s *server) createUniversalSingleMessage(conv *Conversation, m *manifest.Manifest) *Message {
	...
	content := fmt.Sprintf("Sesión aislada de %s lista. Envíame una instrucción en lenguaje natural o usa /<comando>. Comandos disponibles: %s.", m.Name, strings.Join(commands, ", "))
	return &Message{
		ID:        generateMessageID(),
		Role:      "framework",
		Framework: m.Name,
		Content:   content,
		Reasoning: "Wrapper universal: el framework no participa del chain conversacional; se adaptará cada mensaje a comandos declarados en framework.manifest.json vía Channel.",
		Status:    "needs_input",
		Events: []MessageEvent{{
			Type:      "framework.needs_input",
			Framework: m.Name,
			Message:   "universal wrapper ready",
		}},
		Timestamp: time.Now(),
	}
}
...
func (s *server) runUniversalSingle(ctx context.Context, ch *adapter.Client, conv *Conversation, m *manifest.Manifest, input string, resources []MessageResource) (universalSingleResult, error) {
	...
	resp, err := ch.ExecuteCommand(ctx, m.Binary.Command, fullArgs, cwd)
	if err != nil {
		return universalSingleResult{}, err
	}
